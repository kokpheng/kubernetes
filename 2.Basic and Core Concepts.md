...
[deployment.yaml](source-codes/Basic%20and%20Core%20Concepts/Labels%20%26%20Selectors/deployment.yaml). We added `nodeSelector` and `sotrageType` in this file.
  ```yaml
...
[deployment.yaml](source-codes/Basic%20and%20Core%20Concepts/Health%20Checks/deployment.yaml). We added `livenessProbe` and `readinessProbe` in this file.
```yaml
apiVersion: apps/v1beta2
kind: Deployment
metadata:
